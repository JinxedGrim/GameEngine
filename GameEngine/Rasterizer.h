...
namespace TerraPGE::Renderer
{
	...
	template<typename T>
	void __fastcall BaryCentricRasterizer(float* DepthBuffer, GdiPP* Gdi, T&& Shader, ShaderArgs* BaseArgs1)
	{
		...
		const Vec3& v0 = Tri->Points[0];
		const Vec3& v1 = Tri->Points[1];
		const Vec3& v2 = Tri->Points[2];

		// Extract per-vertex attributes
		Vec3 uvw0 = Tri->TexCoords[0].AsVec3(), uvw1 = Tri->TexCoords[1].AsVec3(), uvw2 = Tri->TexCoords[2].AsVec3();
		Vec3 n0 = Tri->FaceNormal, n1 = Tri->FaceNormal, n2 = Tri->FaceNormal;
		...
		for (int y = minY; y <= maxY; y++)
		{
			for (int x = minX; x <= maxX; x++)
			{
				...
				Vec4 InterpolatedPos = Vec4(Tri->WorldSpaceVerts[0] * alpha + Tri->WorldSpaceVerts[1] * beta + Tri->WorldSpaceVerts[2] * gamma, 1.0f);
				...
				if (Depth < DepthBuffer[idx])
				{
					...
					else if (*ShaderType != ShaderTypes::SHADER_FRAGMENT || Tri->OverrideTextureColor || !Core::DoLighting)
					{
						...
						{
							Vec3 TexturCol = Tri->Material->Textures.at(0)->GetPixelColor(uvw.x, 1.0f - uvw.y).GetRGB();
	...
	void __fastcall BaryCentricRasterizerDepth(Triangle* Tri, float* Buffer, const SIZE_T BufferWidth, const SIZE_T BufferHeight, const Matrix& Vp)
	{
		// Allocating now reduces copying
		float FarSubNear = Core::FFAR - Core::FNEAR;
		float FarNear = Core::FFAR * Core::FNEAR;
